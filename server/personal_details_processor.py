...
class PersonalDetailsProcessor:
    ...
    def process_message(input_message,refactored_message,llm,db):
        ...
        query_examples = [
        {
            "input": "Can u provide the details of Manish Pandey ",
            "query": """select REPLACE(TRIM(CONCAT(pd.given_name, ' ', pd.middle_name, ' ', pd.family_name)), '  ', ' ') AS patient_name, pd.gender,pd.age as patient_age,bpad.bed_number as patient_bed_number ,pa.kid as cure_kid from person_details_default pd
                        LEFT JOIN person_attributes pa on pa.person_id=pd.person_id 
                        LEFT JOIN bed_patient_assignment_default bpad  on bpad.patient_id=pd.person_id WHERE REPLACE(TRIM(CONCAT(pd.given_name, ' ', pd.middle_name, ' ', pd.family_name)), '  ', ' ') ILIKE '%Manish Pandey%';
                """
        },
        {
            "input": "details of patient Kishore",
            "query": """select REPLACE(TRIM(CONCAT(pd.given_name, ' ', pd.middle_name, ' ', pd.family_name)), '  ', ' ') AS patient_name, pd.gender,pd.age as patient_age,bpad.bed_number as patient_bed_number ,pa.kid as cure_kid from person_details_default pd
                        LEFT JOIN person_attributes pa on pa.person_id=pd.person_id 
                        LEFT JOIN bed_patient_assignment_default bpad  on bpad.patient_id=pd.person_id WHERE REPLACE(TRIM(CONCAT(pd.given_name, ' ', pd.middle_name, ' ', pd.family_name)), '  ', ' ') ILIKE '%Kishore%';
                """
        },
        {
            "input": "Give me the details of patient in bed ICU2",
            "query": """select REPLACE(TRIM(CONCAT(pd.given_name, ' ', pd.middle_name, ' ', pd.family_name)), '  ', ' ') AS patient_name, pd.gender,pd.age as patient_age,bpad.bed_number as patient_bed_number ,pa.kid as cure_kid from person_details_default pd
                        LEFT JOIN person_attributes pa on pa.person_id=pd.person_id 
                        LEFT JOIN bed_patient_assignment_default bpad  on bpad.patient_id=pd.person_id WHERE bpad.bed_number = 'ICU2' and bpad.date_stopped IS NULL;
                """
        }
        ]
        ...
        answer_prompt = PromptTemplate.from_template(
             """Given the following user question, corresponding SQL query, and SQL result, answer the user question . Carefully follow the below rules while framing your answer .
                 - If there are no rows in SQL Result , Feel free to say  'Sorry! I’m not able to fetch patient details for you right now'
                 - If SQL result has either only 1 row or more than one row but patient name in all the rows are same  . Go ahead and provide the name & details for the patient " 
                 - If there are more than 1 distinct patient name in SQL Result ,Say Aah ! & politely say that there are more than one patient for your inquiry & return all of their name , age & bednumber alone & ask for which patient they need details " 
                 - If any column values asked is null , just say Not available " 
                 - For boolean column values , for true or false Say Yes or No respectively
            Question: {question}
